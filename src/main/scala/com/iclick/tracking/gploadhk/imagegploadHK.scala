...
object imagegploadHK extends Loggable{
  def togp(cmd1: String, cmd2: String,contains_str:String) = {
    ...
    set.foreach {
      x =>
       ...
       try{
       val cmd=s"""psql -h "${FileCmd.GPHOST}" -U "${FileCmd.GPUSER}"  "${FileCmd.GPDB}" -f image_hd_to_gp_hk.sql -v batchid='${batch_id}' -v hdfsloc='${hdfsloc}'"""
