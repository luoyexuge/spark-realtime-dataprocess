...
*/
trait Loggable {
  private def logger() = Loggable.getLogger(getClass().getName())
...
object Loggable {
  val loggers = new HashMap[String, Logger]
  def getLogger(name: String) = loggers.getOrElseUpdate(name, Logger.getLogger(name))
}